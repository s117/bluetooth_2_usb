...
SERVICE_NAME="bluetooth_2_usb"; INSTALL_DIR="/opt/bluetooth_2_usb"; VENV_DIR="/opt/bluetooth_2_usb/venv"; VERBOSE=0; EXIT_CODE=0
while [[ $# -gt 0 ]]; do case "$1" in --service) SERVICE_NAME="$2"; shift 2;; --dir) INSTALL_DIR="$2"; shift 2;; --venv) VENV_DIR="$2"; shift 2;; --verbose) VERBOSE=1; shift;; -h|--help) cat <<EOF
Usage: sudo bash smoke_test.sh [--service bluetooth_2_usb] [--dir /opt/bluetooth_2_usb] [--venv /opt/bluetooth_2_usb/venv] [--verbose]
EOF
exit 0;; *) FAIL "Unknown option: $1"; exit 2;; esac; done

BOOT_DIR="/boot/firmware"; [[ -d "$BOOT_DIR" ]] || BOOT_DIR="/boot"; CONFIG_TXT="${BOOT_DIR}/config.txt"; CMDLINE_TXT="${BOOT_DIR}/cmdline.txt"
LOG_DIR="/var/log/bluetooth_2_usb"; mkdir -p "$LOG_DIR"; REPORT="${LOG_DIR}/smoke_$(date +%Y%m%d_%H%M%S).txt"
